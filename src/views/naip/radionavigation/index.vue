<template>
    <div class="container">
        <Breadcrumb v-show="false" :items="['menu.source', 'menu.radionavigation']" />
        <div class="content">
            <div class="content-main">
                <tiny-layout style="width: 100%">
                    <tiny-row>
                        ...
                        <tiny-col :span="8.5">
                            <radionavigationForm :formData="formData" />
                        </tiny-col>
                    </tiny-row>
                </tiny-layout>
            </div>
...
import { queryRadioNavigationTree, queryRadioNavigationDetail } from '@/api/fetchInterface';
import radionavigationForm from './components/radionavigationForm.vue';
...
const getNodeDataSync = async () => {
    const { data } = await queryRadioNavigationTree();
    return data;
}
...
const fetchData = async (ID: string) => {
    ...
    try {
        if (ID && ID !== '0') {
            const { data } = await queryRadioNavigationDetail({ id: ID || '0' });
