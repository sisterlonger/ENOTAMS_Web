    ...
    <div class="content">
      <div class="content-main">
        ...
        <tiny-collapse v-model="activeNames">
          <tiny-collapse-item title="查询" name="0">
            <tiny-form label-width="100px" label-position="right" class="filter-form" size="small">
              <tiny-row>
                <tiny-col :span="4">
                  <tiny-form-item label="校验名">
                    <tiny-input v-model="formData.validationName" placeholder="请输入校验名" clearable></tiny-input>
                  </tiny-form-item>
                </tiny-col>
                <tiny-col :span="4">
        ...
        <tiny-grid ref="gridRef" :fetch-data="fetchData" seq-serial :pager="pagerConfig"
          ...
          <tiny-grid-column field="validationName" title="校验名"></tiny-grid-column>
...
async function getData({ page }) {
  ...
  let response = await queryValidationList(formData.value);
...
const editRowEvent = (row) => {
  validationID.value = row.validationID;
...
async function toolbarButtonClickEvent({ code, $grid }) {
  switch (code) {
    ...
    case 'deleteSelection': {
      let ids = $grid.getSelectRecords().map((item) => { return item.validationID });
      await deleteValidation(ids);
      Modal.message({
        message: '删除成功!',
        status: 'success',
      });
