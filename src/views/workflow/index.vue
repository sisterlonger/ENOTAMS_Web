<template>
    <div class="tiny-demo">
        <iframe src="http://localhost:1024/" width="100%" height="900px">
            <p>您的浏览器不支持iframe标签。</p>
        </iframe>
        <button @click="toggleStatus">切换状态</button>
        <tiny-flowchart ref="chart" :data="chartData" :config="chartConfig" @click-node="onClickNode"
            @click-link="onClickLink" @click-blank="onClickBlank">
...
const HandlerDropdown = {
    ...
    methods: {
        ...
        iconStyle(handler) {
            ...
            return (
                `${this.imgStyle}display:flex;align-items:center;justify-content:center;border-width:1px;border-style:solid;color`
            )
        }
    }
}
...
chartConfig.headUrl = `https://res.hc-cdn.com/tiny-vue-web-doc/3.20.2.20241210164611/static/images/mountain.png`

chartConfig.checkItemStatus = (item) => ['已转审', '已同意'].includes(item.status)
chartConfig.adjustPos = (afterNode) => {
    if (afterNode.raw.name === '2') {
        afterNode.y += 1;
    }
};
